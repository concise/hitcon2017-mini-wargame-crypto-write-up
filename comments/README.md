...
- 這道題目用到的密碼學知識不多，主要是 [block cipher][bc] 和 [block cipher
...
- 觀察 prob.py 程式數個可被利用的弱點：
  ...
  4.  一旦成功建立連線，server 立刻就會送我們一組明文密文 pair
  5.  程式片段 `send_msg('command not found')` 被執行與否可以被觀察出來
  6.  只要一個 block 的 byte 0、byte 1、byte 2 已知，我們就可以利用 `echo` 指令
      1.  做 chosen ciphertext attack 去猜一個 block 加密前的 byte 15
      2.  做 chosen ciphertext attack 去猜一個 block 加密前的 byte 3 到 byte 14
...
- 不同但相關的 [padding oracle attack][poa] 十分經典，這些看似很傻的弱點有不少實際案例
